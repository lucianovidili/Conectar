  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (cells.length > 0) { // Ignorar el mensaje "sin resultados"
      for (let j = 0; j < cells.length; j++) {
        ...
        // Dividir en palabras y buscar frases completas
        const words = cellText.split(/\s+/);
        const textareaWords = textareaContent.split(/\s+/);

        if (
          words.join(" ").includes(filter) || // Verificar en el contenido del <td>
          textareaWords.join(" ").includes(filter) // Verificar en el <textarea>
        ) {
          rowMatches = true;
          break;
        }
